    ...
    for data in datas["data"]["datas"]:
        # 过滤jpg file
            try:
                cursor = conn.cursor()
                # 创建sql语句
                sql = 'insert into station (start_station_name,end_station_name,start_time,arrive_time,tz_num,zy_num,ze_num,lishi,station_train_code) values ("%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (
